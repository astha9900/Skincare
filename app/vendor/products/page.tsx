...
interface Product {
  ...
  discount: number | null; inStock: boolean; stockCount: number; image: string
...
export default function VendorProductsPage() {
  ...
  const vendorBrand = (session?.user as { vendorBrand?: string })?.vendorBrand ?? ''

  const load = useCallback(async () => {
    setLoading(true)
    try {
      const params = new URLSearchParams({ limit: '100', vendor: 'me' })
  ...
  const EMPTY = {
    name: '', brand: vendorBrand, category: '', description: '',
    price: 0, originalPrice: null, discount: null,
    inStock: true, stockCount: 100, image: '', ingredients: '', howToUse: '',
  }

  function openNew() { setEditing(EMPTY); setIsNew(true); setDialogOpen(true) }
  function openEdit(p: Product) { setEditing({ ...p }); setIsNew(false); setDialogOpen(true) }

  async function handleDelete(id: string) {
    if (!confirm('Delete this product?')) return
    const r = await fetch(`/api/products/${id}`, { method: 'DELETE' })
    if (r.ok) { toast({ title: 'Deleted' }); load() }
    else { const d = await r.json(); toast({ title: 'Error', description: formatApiError(d.error), variant: 'destructive' }) }
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      const r = await fetch(url, {
        ...
        body: JSON.stringify({ ...editing, brand: vendorBrand || editing.brand }),
      })
      const d = await r.json()
      if (!r.ok) { toast({ title: 'Error', description: formatApiError(d.error, 'Failed'), variant: 'destructive' }); return }
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <Button onClick={openNew} className="bg-brand-primary hover:bg-brand-primary/90 text-white">Add your first product</Button>
        </div>
      ) : (
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2 flex items-center gap-3">
                <Switch checked={editing.inStock ?? true} onCheckedChange={v => set('inStock', v)} id="inStockV" />
                <Label htmlFor="inStockV">In Stock</Label>
              </div>
            </div>
